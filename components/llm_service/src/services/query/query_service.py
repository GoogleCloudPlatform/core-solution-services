...
async def query_generate_for_chat(
            user_id: str,
            prompt: str,
            q_engine: QueryEngine,
            user_data: Optional[dict] = None,
            rank_sentences=False,
            query_filter: Optional[dict]=None) -> \
                Tuple[List[QueryReference], List[DataSourceFile]]:
  """
  Execute a query over a query engine and return references and context files.
  Unlike query_generate(), this function does not generate a response since
  it will be used within chat generation.

  Args:
    user_id: user id of user making query
    prompt: the text prompt to pass to the query engine
    q_engine: the query engine to use
    user_data (optional): dict of user data from auth token
    rank_sentences: (optional): rank sentences in retrieved chunks
    query_filter: (optional): dict of key value pairs for filtering results
  ...
  Returns:
    ...
    List of DataSourceFile objects for multimodal content

  Raises:
    ResourceNotFoundException if the query engine doesn't exist
  """
  Logger.info(f"Executing chat query: "
              f"user_id=[{user_id}], "
              f"prompt=[{prompt}], q_engine=[{q_engine.name}]")
  ...
  if not query_filter:
    query_filter = None
  Logger.info(f"query_generate_for_chat query filter = {query_filter}")
  ...
  # Extract multimodal content from references
  context_files = []
  for ref in query_references:
    if ref.modality != "text" and ref.chunk_url:
      ref_filename = ref.chunk_url
  ...
  return query_references, context_files
