...
def create_query_doc_chunks_text(firestore_emulator, clean_firestore):
  ...
  #SC240619: TO DO: Create qdoc_chunk8_text out of QUERY_DOCUMENT_CHUNK_EXAMPLE_8_TEXT
  ...
  #SC240619: TO DO: Create qdoc_chunk9_text out of QUERY_DOCUMENT_CHUNK_EXAMPLE_9_TEXT
...
class FakeDataSource(DataSource):
  ...
  def chunk_document(self, doc_name: str, doc_url: str,
    ...
    if doc_url == QUERY_DOCUMENT_EXAMPLE_1["doc_url"]:
      ...
      chunk_list = None
    return chunk_list #SC240619: TO DO: Why are we returning a string, instead of a list of strings?
  ...
  def chunk_document_multi(self, doc_name:str, doc_url: str,
    ...
    if doc_url == QUERY_DOCUMENT_EXAMPLE_1["doc_url"]:
      ...
      chunk_list = None
    return chunk_list #SC240619: TO DO: Why are we returning a dict, instead of a list of dicts?
  #SC240619: TO DO: In definition of chunk_document_multi, shouldn't it return a List[dict], not a List[str]?
...
def test_query_engine_build_multi(mock_get_vector_store, mock_build_doc_index,
  ...
  doc_url = FAKE_GCS_PATH
  q_engine, docs_processed, docs_not_processed = \
      query_engine_build(doc_url, QUERY_ENGINE_EXAMPLE_MULTI["name"], create_user.id,
                         None, None, None, None, None,
                         QUERY_ENGINE_EXAMPLE_MULTI["params"]) #SC240619: DONE: Is this a real query engine somewhere?  Which project?  No it is not real
  assert q_engine.created_by == create_user.id
  assert q_engine.name == QUERY_ENGINE_EXAMPLE_MULTI["name"]
  assert q_engine.doc_url == doc_url
  assert q_engine.params["is_multimodal"].lower() == QUERY_ENGINE_EXAMPLE_MULTI["params"]["is_multimodal"].lower() #QUERY_ENGINE_EXAMPLE_MULTI["params"]["is_multimodal"] #SC240619: TO DO: Make is_multimodal be a field of params dict
  assert docs_processed == [create_query_docs[0], create_query_docs[1]]
  assert docs_not_processed == [create_query_docs[2]]
...
def test_query_engine_build_textonly(mock_get_vector_store, mock_build_doc_index,
  ...
  doc_url = FAKE_GCS_PATH
  q_engine, docs_processed, docs_not_processed = \
      query_engine_build(doc_url, QUERY_ENGINE_EXAMPLE_TEXTONLY["name"], create_user.id,
                         None, None, None, None, None,
                         QUERY_ENGINE_EXAMPLE_TEXTONLY["params"]) #SC240619: DONE: Is this a real query engine somewhere?  Which project?  No it is not real
  assert q_engine.created_by == create_user.id
  assert q_engine.name == QUERY_ENGINE_EXAMPLE_TEXTONLY["name"]
  assert q_engine.doc_url == doc_url
  assert q_engine.params["is_multimodal"].lower() == QUERY_ENGINE_EXAMPLE_TEXTONLY["params"]["is_multimodal"].lower() #SC240619: TO DO: Make is_multimodal be a field of params dict
  assert docs_processed == [create_query_docs[0], create_query_docs[1]]
  assert docs_not_processed == [create_query_docs[2]]
...
def test_build_doc_index(mock_process_documents, create_engine,
  ...
  with mock.patch("google.cloud.storage.Client"):
    docs_processed, docs_not_processed = \
        build_doc_index(doc_url, create_engine, qe_vector_store) #SC240624: DONE: Do not pass in is_multimodal input at all
  assert docs_processed == [create_query_docs[0], create_query_docs[1]]
  assert docs_not_processed == [create_query_docs[2]]
...
def test_build_doc_index_multi(mock_process_documents, create_engine_multi,
  ...
  with mock.patch("google.cloud.storage.Client"):
    docs_processed, docs_not_processed = \
        build_doc_index(doc_url, create_engine_multi, qe_vector_store, True)
  assert docs_processed == [create_query_docs[0], create_query_docs[1]]
  assert docs_not_processed == [create_query_docs[2]]
...
def test_build_doc_index_textonly(mock_process_documents, create_engine_textonly,
  ...
  with mock.patch("google.cloud.storage.Client"):
    docs_processed, docs_not_processed = \
        build_doc_index(doc_url, create_engine_textonly, qe_vector_store, False)
  assert docs_processed == [create_query_docs[0], create_query_docs[1]]
  assert docs_not_processed == [create_query_docs[2]]
...
def test_process_documents(mock_get_datasource, create_engine):
  ...
  docs_processed, docs_not_processed = \
      process_documents(doc_url, qe_vector_store, create_engine, None) #SC240619: Do not pass in is_multimodal input at all
...
def test_process_documents_multi(mock_get_datasource, create_engine_multi):
  ...
  Path(DSF2.local_path).touch()
  docs_processed, docs_not_processed = \
      process_documents(doc_url, qe_vector_store, create_engine_multi, None, True)
  assert {doc.doc_url for doc in docs_processed} == {DSF1.src_url, DSF2.src_url}
  assert set(docs_not_processed) == {DSF3.src_url}
...
def test_process_documents_textonly(mock_get_datasource, create_engine_textonly):
  ...
  Path(DSF2.local_path).touch()
  docs_processed, docs_not_processed = \
      process_documents(doc_url, qe_vector_store, create_engine_textonly, None, False)
  assert {doc.doc_url for doc in docs_processed} == {DSF1.src_url, DSF2.src_url}
  assert set(docs_not_processed) == {DSF3.src_url}
